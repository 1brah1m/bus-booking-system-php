    ...
    <body>
        ...
        <div class="container">
            <div class="row">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody> 
                            @foreach($buses as $bus)
                                <tr>
                                    <th scope="row"><a href="/buses/{{ $bus->id }}">{{ $bus->id }}</a></th>
                                    <td>{{ $bus->company }}</td>
                                    <td>{{ $bus->start_location }}</td>
                                    <td>{{ $bus->end_location }}</td>
                                    <td>{{ date_format(new DateTime($bus->start_time), 'H:i') }}</td>
                                    <td>{{ date_format(new DateTime($bus->end_time), 'H:i') }}</td>
                                    <td>{{ $bus->stops }}</td>
                                    <td>{{ $bus->price }}</td>
                                </tr>
                            @endforeach
                ...
                <div class="col-2">
                    <a href="/buses/create" class="btn btn-primary">New Bus</a>
                </div>
            </div>
        </div>
